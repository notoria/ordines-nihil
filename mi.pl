...
initialize_builtin(Bs) :-
    ...
    list_foldl(
        ...
        [   % avlt_insert('$op'/3, '$op'(_A,_B,_C)-'@op'(_A,_B,_C)),
            ...
            avlt_insert(atom/1, atom(_A)-'$atom'(_A)),
            avlt_insert((=)/2, =(_A,_B)- unify_with_occurs_check(_A,_B)),
...
%     solve(T-Gs=T1-Gs1, S0, S).
solve_special(findall(T, G_0, Ts0, Ts), S0, S) :-
    findall(T, solve(G_0, S0, _), Us0, Us),
    solve(Ts0-Ts=Us0-Us, S0, S).
...
'@equations'(_Es, _Es0, X, Y) -->
    { X == Y }, !.
'@equations'(Es, _Es0, X, Y) -->
